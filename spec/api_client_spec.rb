    ...
    context 'URL config' do
      ...
      context "creates a valid authorization_url" do
        it "passes through attributes" do
          ...
          expect(api_client.authorization_url).to eq('https://identity.xero.com/identity/connect/authorize?response_type=code&client_id=abc&redirect_uri=https%3A%2F%2Fmydomain.com%2Fcallback&scope=openid+profile+email+accounting.transactions+accounting.settings&state=i-am-customer-state')
        end
        ...
        it "Does not append state if it is not provided" do
          ...
          api_client = XeroRuby::ApiClient.new(credentials: creds)
          expect(api_client.authorization_url).to eq('https://identity.xero.com/identity/connect/authorize?response_type=code&client_id=abc&redirect_uri=https%3A%2F%2Fmydomain.com%2Fcallback&scope=openid+profile+email+accounting.transactions+accounting.settings')
        end
